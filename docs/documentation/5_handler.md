...
- `on` - class attribute, instance of `On` for registering triggers  
...
- `message` - instance attribute, `Message` object from the Telegram API
...
- `delete_user_initial_message(self)` - deletes the user's original message that initialized the chain  
